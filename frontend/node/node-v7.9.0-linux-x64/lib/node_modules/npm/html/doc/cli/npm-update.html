...
aliases: up, upgrade
  ...
  <p>As of <code>npm@2.6.1</code>, the <code>npm update</code> will only inspect top-level
    packages.
  ...
  <p>For the examples below, assume that the current package is <code>app</code> and it depends
    on dependencies, <code>dep1</code> (<code>dep2</code>, .. etc.). The published versions of
    <code>dep1</code> are:</p>
  <pre><code>{
  ...
  <p>Then <code>npm update</code> will install <code>dep1@1.2.2</code>, because <code>1.2.2</code>
    is <code>latest</code> and
  ...
  <p>In this case, running <code>npm update</code> will install <code>dep1@1.1.2</code>. Even
    though the <code>latest</code>
    tag points to <code>1.2.2</code>, this version does not satisfy <code>~1.1.1</code>, which is
    equivalent
    to <code>&gt;=1.1.1 &lt;1.2.0</code>. So the highest-sorting version that satisfies <code>~1.1.1</code>
    is used,
    which is <code>1.1.2</code>.</p>
  <h3 id="caret-dependencies-below-1-0-0">Caret Dependencies below 1.0.0</h3>
  <p>Suppose <code>app</code> has a caret dependency on a version below <code>1.0.0</code>, for
    example:</p>
  <pre><code>&quot;dependencies&quot;: {
  &quot;dep1&quot;: &quot;^0.2.0&quot;
}
  ...
  <p>Then <code>npm update</code> will install <code>dep1@0.4.1</code>, because that is the
    highest-sorting
  ...
  <p>Then <code>npm update --save</code> will install <code>dep1@1.2.2</code> (i.e.,
    <code>latest</code>),
  ...
  <p>Note that <code>npm</code> will only write an updated version to <code>package.json</code>
    if it installs a new package.</p>
  <h3 id="updating-globally-installed-packages">Updating Globally-Installed Packages</h3>
  <p><code>npm update -g</code> will apply the <code>update</code> action to each globally
    installed
